...
const CartScreen = () => {
  ...
  const checkoutHandler = async () => {
    ...
    const confirmPayment = window.confirm('Do you want to proceed to pay?');

    if (confirmPayment) {
      try {
        // Call the createOrder mutation
        await createOrder({ orderItems: cartItems }).unwrap();

        // Show success message
        alert('Thank you for purchasing your products!');

        // Clear cart
        dispatch(clearCartItems());

        // Optional: redirect to My Orders page
        navigate('/myorders');
      } catch (err) {
          ...
          {cartItems.length === 0 ? (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
              {cartItems.map((item) => (
                <div
                  key={item._id}
                  className="flex items-center border-b dark:border-gray-700 p-4 last:border-b-0"
                >
                  {/* Image */}
                  <img
                    src={item.image}
                  ...
                  <div className="flex-1">
                    <Link
                      to={`/product/${item._id}`}
                      className="text-lg font-medium text-gray-800 dark:text-white hover:text-blue-600"
                    >
                      {item.name}
                    </Link>
                  </div>
